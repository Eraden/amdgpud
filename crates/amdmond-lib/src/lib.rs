...
impl AmdMon {
    ...
    /// Returns name and gpu temperature percent of maximum GPU temperature
    /// range
    ...
    pub fn gpu_usage(&self) -> Result<f64> {
        if let Some(input) = self.temp_input.as_ref() {
            let value = self.read_gpu_usage(&input.as_string())?;
    ...
    pub fn gpu_usage_of(&self, input_idx: usize) -> Option<(&String, Result<f64>)> {
        self.inputs.get(input_idx).map(|name| {
            let usage = self.read_gpu_usage(name).map(|usage| usage as f64 / 100f64);
            (name, usage)
        })
    }
