...
use amdgpu_config::fan::{TempPoint, DEFAULT_FAN_CONFIG_PATH};
        ...
        match key.code {
            ...
            KeyCode::Down => match app.selected_point {
                ...
                None => match app.table_state.selected() {
                    Some(index) => {
                        if index + 1 < app.config.temp_matrix().len() {
...
where
    Read: FnOnce(&TempPoint) -> f64 + Copy,
    Write: FnOnce(&mut TempPoint) -> &mut f64,
{
    let prev = index
        .checked_sub(1)
        .and_then(|i| app.config.temp_matrix().get(i))
        .map(read)
        .unwrap_or(0.0);
    let next = app
        .config
...
fn y_mut(p: &mut TempPoint) -> &mut f64 {
    &mut p.speed
}

fn x_mut(p: &mut TempPoint) -> &mut f64 {
    &mut p.temp
}

fn y(p: &TempPoint) -> f64 {
    p.speed
}

fn x(p: &TempPoint) -> f64 {
    p.temp
}
